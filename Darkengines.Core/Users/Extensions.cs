...
namespace Darkengines.Core.Users {
	public static class Extensions {
		...
		public static ModelBuilder ConfigureUsers(this ModelBuilder modelBuilder) {
			...
			userEmailAddressTypeEntityBuilder.HasKey(userEmailAddress => new { userEmailAddress.UserId, userEmailAddress.EmailAddress });
			userEmailAddressTypeEntityBuilder.Property(user => user.EmailAddress).HasMaxLength(256).IsRequired();
			userEmailAddressTypeEntityBuilder.Property(user => user.HashedEmailAddress).IsRequired();
			userEmailAddressTypeEntityBuilder.HasIndex(user => user.HashedEmailAddress).IsUnique();
